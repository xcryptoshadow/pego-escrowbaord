...
export const EscrowItem = ({
	...
	const { signerAddress } = useContext(EscrowContext);
	...
	const QrcodeImage = useMemo(() => {
		...
		value += `\nID: ${escrow.id}`
		value += `\nAmount: ${ethers.utils.formatUnits(escrow.amount, decimals??18)} ${symbol}`
		value += `\nFee: ${ethers.utils.formatUnits(escrow.fee, decimals??18)} ${symbol}`
		value += `\nFrom: ${escrow.buyer}`
		value += `\nTo: ${escrow.seller}`
	...
	return (
		...
		>
			...
			<h3>
				{ethers.utils.formatUnits(escrow.amount, decimals??18)} {symbol}
			</h3>
			...
			<ul className="list-group mt-3">
				...
				<li className="list-group-item d-flex justify-content-between">
					...
					<span>
						{ethers.utils.formatUnits(escrow.fee, decimals??18)} {symbol}
					</span>
				</li>
				{metaLoading ? (
					<Skeleton />
